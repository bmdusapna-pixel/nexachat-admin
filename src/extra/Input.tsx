    ...
    <>
      ...
      >
        {label && (
          <label
            htmlFor={id}
            className={`${(types === "radio" || types === "checkbox") &&
...
const ImageScript : any = (value: any, setMultiImagePath: any) => {
  useEffect(() => {
      ...
      <div className={`selectMain`}>
        ...
        >
          <p className={`m0 m10-right showSelectValue`}>
            <p
              className={`${!key ? "text-gray" : "text-dark"
        ...
        >
          ...
          <p className={`m0 m10-right showSelectValue`}>
            {selectedValues?.length === 0 ? (
              ...
              selectedValues.map((selectedValue : any, index : number) => {
                const optionObject =
...
const SelectScript = ({ value , id, angle } : any) => {
  useEffect(() => {
    $(".dropMenu").hide();
    const handleDrop = () => {
